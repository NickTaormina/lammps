...
NPairCAC::~NPairCAC()
{
  ...
  if (neigh_allocated) {
    for (int init = 0; init < max_atom_count; init++) memory->destroy(list_container[init]);
    memory->sfree(list_container);
  }
  if (quadrature_point_max) memory->destroy(quadrature_point_data);
  if (nmax) memory->destroy(quadrature_counts);
  if (quad_allocated) {
...
void NPairCAC::build(NeighList *list)
{
  ...
  for (int init = 0; init < mbins; init++) { bin_scan_flags[init] = 0; }

  // initialize or grow memory for the neighbor list of virtual atoms at quadrature points
  if (atom->nlocal) allocate_neigh_list();
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    if (nbin_element_overlap[i] > MAXBIN)
      error->one(FLERR, " too many bin overlaps for one element; simulation may be unstable");
        ...
        for (int jj = 0; jj < bin_ncontent[ibin + stencil[k]]; jj++) {
          ...
          if (neighbor_element_type != 0 || current_element_type != 0) {
            if (neighbor_element_type != 0 && current_element_type != 0) {
              ...
              if (CAC_decide_element2element(i, j)) list_container[i][neigh_count++] = j;
            }
          } else {
            delx = x[i][0] - x[j][0];
            dely = x[i][1] - x[j][1];
            ...
            if (rsq <= cutneighmax * cutneighmax) list_container[i][neigh_count++] = j;
          }
        }
      }
      //reset bin scan flags
      for (int reset = 0; reset < nbins_searched; reset++) bin_scan_flags[bins_searched[reset]] = 0;
    }
    numneigh[i] = neigh_count;

    //reset scan flags for next quadrture point searching for its neighbors
    for (int reset = 0; reset < neigh_count; reset++) { scan_flags[list_container[i][reset]] = 0; }
  }
  ...
  if (atom->interface_quadrature) {
    for (i = 0; i < nlocal; i++) {
      if (!(element_type[i] == 1 || element_type[i] == 3)) continue;
      for (int jj = 0; jj < numneigh[i]; jj++) {
        ...
        if (element_scale[i][0] * element_scale[i][1] * element_scale[i][2] >
            2 * element_scale[j][0] * element_scale[j][1] * element_scale[j][2]) {
          interface_flags[i] = 1;
          break;
        }
      }
  ...
  if (ghost_quad_flag) comm->forward_comm_npair(this, 0);

  if (nsum > nmax) allocate_local_arrays();
    ...
    if (element_type[i] != 0) {
      for (poly_counter = 0; poly_counter < poly_count[i]; poly_counter++) {
        ...
        compute_surface_depths(interior_scale[0], interior_scale[1], interior_scale[2],
                               poly_surface_count[0], poly_surface_count[1], poly_surface_count[2],
                               i, element_type[i]);
        if (poly_counter == 0) {
      ...
      if (surface_counts[i][0] > surface_counts_max[0])
        surface_counts_max[0] = surface_counts[i][0];
      if (surface_counts[i][1] > surface_counts_max[1])
        surface_counts_max[1] = surface_counts[i][1];
      if (surface_counts[i][2] > surface_counts_max[2])
        surface_counts_max[2] = surface_counts[i][2];
    }
  }
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    for (int iquad = 0; iquad < quadrature_counts[i]; iquad++) {
      for (poly_counter = 0; poly_counter < poly_count[i]; poly_counter++) {
        ...
        //Q8 case
        if (current_element_type == 1 || current_element_type == 3) {
        ...
        } else if (current_element_type == 0) {    //atom case
        ...
        for (k = 0; k < nstencil; k++) {
          for (int jj = 0; jj < bin_ncontent[ibin + stencil[k]]; jj++) {
            ...
            if (neigh_count == quad_scan_list_max) {
              ...
              memory->grow(current_quad_list, quad_scan_list_max, "NPair CAC:current_quad_list");
            }
            ...
            if (neighbor_element_type != 0) {
              ...
              rsq = delx * delx + dely * dely + delz * delz;
              if (rsq <= cutneighmax * cutneighmax) current_quad_list[neigh_count++] = j;
            }
          }
        }

        //use the current quad list to construct the virtual atom list with asa cg and local unit cell scans
        quad_list_build(i, quadrature_point_data[qi][0], quadrature_point_data[qi][1],
                        quadrature_point_data[qi][2]);
...
double NPairCAC::shape_function_derivative(double s, double t, double w, int flag, int index,
                                           int derivative)
{
...
void NPairCAC::quad_list_build(int iii, double s, double t, double w)
{
  ...
  if (current_element_type == 1 || current_element_type == 3) {
    ...
    if (outer_neigh_flag) {
      rcut = 2 * cut_global + cutoff_skin + cut_add;
    } else {
      rcut = cut_global + cutoff_skin + cut_add;
    }
    ...
    for (int id = 0; id < 3; id++) {
      for (int jd = 0; jd < 3; jd++) {
        ...
        for (int n = 0; n < nodes_per_element; n++) {
          boxmap_matrix[id][jd] +=
              current_nodal_positions[n][id] * shape_function_derivative(s, t, w, 2, n + 1, jd + 1);
        }
      }
    }
    ...
    double norm_b_orth =
        sqrt(b_orth[0] * b_orth[0] + b_orth[1] * b_orth[1] + b_orth[2] * b_orth[2]);
    ...
    double norm_c_orth =
        sqrt(c_orth[0] * c_orth[0] + c_orth[1] * c_orth[1] + c_orth[2] * c_orth[2]);
      ...
      for (int wcount = -w_span; wcount < w_span + 1; wcount++) {
        ...
        for (int tcount = t_lower_limit; tcount < t_upper_limit + 1; tcount++) {
          ...
          for (int scount = s_lower_limit; scount < s_upper_limit + 1; scount++) {
            ...
            outofbounds = 0;
            if (((scount == 0 && tcount == 0) && wcount == 0) && polyscan == poly_counter) {
              continue;
            }
            ...
            if (outofbounds == 0) {
              for (int kk = 0; kk < nodes_per_element; kk++) {
                shape_func2 = shape_function(scanning_unit_cell[0], scanning_unit_cell[1],
                                             scanning_unit_cell[2], 2, kk + 1);
              ...
              if (distancesq < (cut_global + cutoff_skin) * (cut_global + cutoff_skin)) {
                if (inner_neigh_index == inner_quad_neigh_maxes[pqi]) {
                  ...
                  memory->grow(inner_quad_lists_ucell[pqi], inner_quad_neigh_maxes[pqi], 3,
                               "NPair CAC:cell coords expand");
                  if (sector_flag)
                    memory->grow(inner_quad_lists_index[pqi], inner_quad_neigh_maxes[pqi], 3,
                                 "NPair CAC:cell indexes expand");
                  else
                    memory->grow(inner_quad_lists_index[pqi], inner_quad_neigh_maxes[pqi], 2,
                                 "NPair CAC:cell indexes expand");
                }
                ...
                if (sector_flag)
                  inner_quad_lists_index[pqi][inner_neigh_index][2] = e2quad_index[iii] +
                      current_poly_count *
                          quad_sector_select(scanning_unit_cell[0], scanning_unit_cell[1],
                                             scanning_unit_cell[2], iii, polyscan) +
                      polyscan;
                ...
                neighbor_weights[iii][1]++;
                         ...
                         (2 * cut_global + cutoff_skin) * (2 * cut_global + cutoff_skin)) {
                if (outer_neigh_flag) {
                  if (outer_neigh_index == outer_quad_neigh_maxes[pqi]) {
                    outer_quad_neigh_maxes[pqi] += EXPAND;

                    memory->grow(outer_quad_lists_ucell[pqi], outer_quad_neigh_maxes[pqi], 3,
                                 "NPair CAC:cell coords expand");
                    if (sector_flag)
                      memory->grow(outer_quad_lists_index[pqi], outer_quad_neigh_maxes[pqi], 3,
                                   "NPair CAC:cell indexes expand");
                    else
                      memory->grow(outer_quad_lists_index[pqi], outer_quad_neigh_maxes[pqi], 2,
                                   "NPair CAC:cell indexes expand");
                  }
                  ...
                  if (sector_flag)
                    outer_quad_lists_index[pqi][outer_neigh_index][2] = e2quad_index[iii] +
                        current_poly_count *
                            quad_sector_select(scanning_unit_cell[0], scanning_unit_cell[1],
                                               scanning_unit_cell[2], iii, polyscan) +
                        polyscan;
              ...
              } else if ((distancesq < (2 * cut_global + cutoff_skin + cut_add) *
                                  (2 * cut_global + cutoff_skin + cut_add) &&
                          outer_neigh_flag) ||
                         (distancesq < (cut_global + cutoff_skin + cut_add) *
                                  (cut_global + cutoff_skin + cut_add) &&
                          !outer_neigh_flag)) {
                if (add_neigh_flag) {
                  if (add_neigh_index == add_quad_neigh_maxes[pqi]) {
                    add_quad_neigh_maxes[pqi] += EXPAND;

                    memory->grow(add_quad_lists_ucell[pqi], add_quad_neigh_maxes[pqi], 3,
                                 "NPair CAC:cell coords expand");
                    if (sector_flag)
                      memory->grow(add_quad_lists_index[pqi], add_quad_neigh_maxes[pqi], 3,
                                   "NPair CAC:cell indexes expand");
                    else
                      memory->grow(add_quad_lists_index[pqi], add_quad_neigh_maxes[pqi], 2,
                                   "NPair CAC:cell indexes expand");
                  }
                  ...
                  if (sector_flag)
                    add_quad_lists_index[pqi][add_neigh_index][2] = e2quad_index[iii] +
                        current_poly_count *
                            quad_sector_select(scanning_unit_cell[0], scanning_unit_cell[1],
                                               scanning_unit_cell[2], iii, polyscan) +
                        polyscan;
    ...
    if (neighborflag == 1) {
      neighbor_accumulate(current_position[0], current_position[1], current_position[2], iii,
                          inner_neigh_index, outer_neigh_index, add_neigh_index);
    }
  } else {
    neighbor_accumulate(current_x[0], current_x[1], current_x[2], iii, inner_neigh_index,
                        outer_neigh_index, add_neigh_index);
  }
...
void NPairCAC::neighbor_accumulate(double x, double y, double z, int iii, int inner_neigh_initial,
                                   int outer_neigh_initial, int add_neigh_initial)
{
  ...
  for (jj = 0; jj < neigh_count; jj++) {
    ...
    neigh_poly_count = poly_count[j];
    check_flag = 0;
    if (neighbor_element_type == 1 || neighbor_element_type == 3) {
      ...
      for (int si = 0; si < 6; si++) {
        ...
        for (int kk = 0; kk < neigh_nodes_per_element; kk++) {
          ...
          min_point[2] += neighbor_element_positions[0][kk][2] *
              shape_func2;    //check for error of minimum point later
        }
        delx = x - min_point[0];
        dely = y - min_point[1];
        delz = z - min_point[2];
      ...
      for (int si = 0; si < 6; si++) {
        if (complete == 0) {
          ...
          iWork[0] = 0;
          iWork[1] = 0;
          for (int Workcounter = 0; Workcounter < 100; Workcounter++) { Work[Workcounter] = 0; }
          ...
          for (poly_min = 0; poly_min < neigh_poly_count; poly_min++) {
            ...
            for (int kk = 0; kk < neigh_nodes_per_element; kk++) {
              ...
              min_point[2] += neighbor_element_positions[poly_min][kk][2] *
                  shape_func2;    //check for error of minimum point later
            }
      ...
      if (complete == 0) {
        if (check_flag)
          error->one(FLERR,
                     "asa_cg iterations failed in finding a solution for quadrature neighboring "
                     "processes. flag = 7");
      }
      ...
      if (complete == 1) {
        ...
        for (int polyscan = 0; polyscan < neigh_poly_count; polyscan++) {
            ...
            for (int jd = 0; jd < 3; jd++) {
              ...
              for (int ni = 0; ni < neigh_nodes_per_element; ni++) {
                ...
                boxmap_matrix[id][jd] += neighbor_element_positions[polyscan][ni][id] *
                    shape_function_derivative(shape_args[0], shape_args[1], shape_args[2], 2,
                                              ni + 1, jd + 1);
              }
            }
          }
          ...
          double norm_b_orth =
              sqrt(b_orth[0] * b_orth[0] + b_orth[1] * b_orth[1] + b_orth[2] * b_orth[2]);
          double proj_c2b_orth =
              (b_orth[0] * c[0] + b_orth[1] * c[1] + b_orth[2] * c[2]) / norm_b_orth;
                ...
                if (outofbounds == 0) {
                  ...
                  if (distancesq < (cut_global + cutoff_skin) * (cut_global + cutoff_skin)) {
                    if (inner_neigh_index == inner_quad_neigh_maxes[pqi]) {
                      ...
                      memory->grow(inner_quad_lists_ucell[pqi], inner_quad_neigh_maxes[pqi], 3,
                                   "Pair CAC:cell coords expand");
                      if (sector_flag)
                        memory->grow(inner_quad_lists_index[pqi], inner_quad_neigh_maxes[pqi], 3,
                                     "Pair CAC:cell indexes expand");
                      else
                        memory->grow(inner_quad_lists_index[pqi], inner_quad_neigh_maxes[pqi], 2,
                                     "Pair CAC:cell indexes expand");
                    }
                    ...
                    inner_quad_lists_index[pqi][inner_neigh_index][1] = polyscan;
                    if (sector_flag)
                      inner_quad_lists_index[pqi][inner_neigh_index][2] = e2quad_index[j] +
                          poly_count[j] *
                              quad_sector_select(scanning_unit_cell[0], scanning_unit_cell[1],
                                                 scanning_unit_cell[2], j, polyscan) +
                          polyscan;
                    ...
                    if (outer_neigh_flag) {
                      ...
                      outer_quad_lists_index[pqi][outer_neigh_index][1] = polyscan;
                      if (sector_flag)
                        outer_quad_lists_index[pqi][outer_neigh_index][2] = e2quad_index[j] +
                            poly_count[j] *
                                quad_sector_select(scanning_unit_cell[0], scanning_unit_cell[1],
                                                   scanning_unit_cell[2], j, polyscan) +
                            polyscan;
                    ...
                    if (add_neigh_flag) {
                      ...
                      add_quad_lists_index[pqi][add_neigh_index][1] = polyscan;
                      if (sector_flag)
                        add_quad_lists_index[pqi][add_neigh_index][2] = e2quad_index[j] +
                            poly_count[j] *
                                quad_sector_select(scanning_unit_cell[0], scanning_unit_cell[1],
                                                   scanning_unit_cell[2], j, polyscan) +
                            polyscan;
      ...
      if (distancesq < (cut_global + cutoff_skin) * (cut_global + cutoff_skin)) {
        ...
        if (outer_neigh_flag) {
          ...
          neighbor_weights[iii][0]++;
                  ...
                  outer_neigh_flag) ||
                 (distancesq <
                      (cut_global + cutoff_skin + cut_add) * (cut_global + cutoff_skin + cut_add) &&
                  !outer_neigh_flag)) {
        if (add_neigh_flag) {
          if (add_neigh_index == add_quad_neigh_maxes[pqi]) {
            add_quad_neigh_maxes[pqi] += EXPAND;
...
void NPairCAC::compute_surface_depths(double &scalex, double &scaley, double &scalez, int &countx,
                                      int &county, int &countz, int eindex, int etype)
{
  double unit_cell_mapped[3];
  ...
  //elements
  if (etype == 1 || etype == 3) {
    unit_cell_mapped[0] = 2 /
        double(current_element_scale[0]);    //element_scale is number of unit cells inside element
    unit_cell_mapped[1] = 2 / double(current_element_scale[1]);
    unit_cell_mapped[2] = 2 / double(current_element_scale[2]);
    ...
    if (ds_surf > 1) { ds_surf = 1; }
    if (dt_surf > 1) { dt_surf = 1; }
    if (dw_surf > 1) { dw_surf = 1; }
    ...
    scaley = 1 - dt_surf;
    scalez = 1 - dw_surf;
    if (ds_surf >= 1) scalex = 0.0;
    if (dt_surf >= 1) scaley = 0.0;
    if (dw_surf >= 1) scalez = 0.0;
...
int NPairCAC::compute_quad_points(int element_index)
{
      ...
      for (int i = 0; i < current_element_scale[0]; i++) {
        for (int j = 0; j < current_element_scale[1]; j++) {
          for (int k = 0; k < current_element_scale[2]; k++) {
            ...
            else if (i == current_element_scale[0] - 1 && j == 0 && k == 0)
              quadrature_point_data[qi][7] = 1;
            else if (i == current_element_scale[0] - 1 && j == current_element_scale[1] - 1 &&
                     k == 0)
              quadrature_point_data[qi][7] = 1;
            else if (i == 0 && j == current_element_scale[1] - 1 && k == 0)
              quadrature_point_data[qi][7] = 1;
            else if (i == 0 && j == 0 && k == current_element_scale[2] - 1)
              quadrature_point_data[qi][7] = 1;
            else if (i == current_element_scale[0] - 1 && j == 0 &&
                     k == current_element_scale[2] - 1)
              quadrature_point_data[qi][7] = 1;
            else if (i == current_element_scale[0] - 1 && j == current_element_scale[1] - 1 &&
                     k == current_element_scale[2] - 1)
              quadrature_point_data[qi][7] = 1;
            else if (i == 0 && j == current_element_scale[1] - 1 &&
                     k == current_element_scale[2] - 1)
              quadrature_point_data[qi][7] = 1;
            else
            ...
            qi += 1;
            if (qi == quadrature_point_max) grow_quad_data();
          }
        }
      }
    } else {
        ...
        for (int i = 0; i < quadrature_node_count; i++) {
          for (int j = 0; j < quadrature_node_count; j++) {
            for (int k = 0; k < quadrature_node_count; k++) {
              ...
              quadrature_point_data[qi][6] = interior_scale[0] * interior_scale[1] *
                  interior_scale[2] * quadrature_weights[i] * quadrature_weights[j] *
          ...
          for (int i = 0; i < isurface_counts[0]; i++) {
            for (int j = 0; j < quadrature_node_count; j++) {
              for (int k = 0; k < quadrature_node_count; k++) {
                ...
                quadrature_point_data[qi][4] = tq;
                quadrature_point_data[qi][5] = wq;
                quadrature_point_data[qi][6] = unit_cell_mapped[0] * interior_scale[1] *
          ...
          for (int i = 0; i < isurface_counts[1]; i++) {
            for (int j = 0; j < quadrature_node_count; j++) {
              for (int k = 0; k < quadrature_node_count; k++) {
                ...
                quadrature_point_data[qi][4] = tq;
                quadrature_point_data[qi][5] = wq;
                quadrature_point_data[qi][6] = unit_cell_mapped[1] * interior_scale[0] *
                    interior_scale[2] * quadrature_weights[j] * quadrature_weights[k];
                quadrature_point_data[qi][7] = 0;
                neigh_quad_counter = neigh_quad_counter + 1;
                qi += 1;
          ...
          for (int i = 0; i < isurface_counts[2]; i++) {
            for (int j = 0; j < quadrature_node_count; j++) {
              for (int k = 0; k < quadrature_node_count; k++) {
                ...
                quadrature_point_data[qi][4] = tq;
                quadrature_point_data[qi][5] = wq;
                quadrature_point_data[qi][6] = unit_cell_mapped[2] * interior_scale[1] *
        ...
        for (int i = 0; i < surface_countsx; i++) {
          for (int j = 0; j < surface_countsy; j++) {
            for (int k = 0; k < quadrature_node_count; k++) {
              ...
              quadrature_point_data[qi][4] = tq;
              quadrature_point_data[qi][5] = wq;
              quadrature_point_data[qi][6] = coefficients =
                  unit_mappedx * unit_mappedy * interior_scalez * quadrature_weights[k];
              quadrature_point_data[qi][7] = 0;
              neigh_quad_counter = neigh_quad_counter + 1;
              qi += 1;
              if (qi == quadrature_point_max) grow_quad_data();
  ...
  } else if (element_type[element_index] == 3) {    //connected element quadrature points
    //check if refined numerical interface is needed
    if (atom->full_quad_flag &&
        interface_flags[element_index]) {    //fullQuadInterface command. not default ig
      //loop over all possible virtual atom sites
      for (int i = 0; i < current_element_scale[0]; i++) {
        for (int j = 0; j < current_element_scale[1]; j++) {
          for (int k = 0; k < current_element_scale[2]; k++) {
      ...
      if (interior_scale[0] > 0 && interior_scale[1] > 0 &&
          interior_scale[2] >
              0) {    //relevant when many unit cells in the element. our interior quad points?
        //printf("quadrature node count: %d\n", quadrature_node_count);
        for (int i = 0; i < quadrature_node_count; i++) {
          for (int j = 0; j < quadrature_node_count; j++) {
            for (int k = 0; k < quadrature_node_count; k++) {
              sq = s = interior_scale[0] * quadrature_abcissae[i];
              ...
              if (tq < 0) signt = -1;
              if (wq < 0) signw = -1;
              s = unit_cell_mapped[0] * (int((s + signs) / unit_cell_mapped[0])) -
                  signs;    //unit cell mapped = 2/element_scale
              t = unit_cell_mapped[1] * (int((t + signt) / unit_cell_mapped[1])) - signt;
              w = unit_cell_mapped[2] * (int((w + signw) / unit_cell_mapped[2])) - signw;
              ...
              //stw is +/- 0.375, 0.375, 0.375 (with surf count 2)
              quadrature_point_data[qi][0] = s;
              quadrature_point_data[qi][1] = t;
              quadrature_point_data[qi][2] = w;
              quadrature_point_data[qi][3] = sq;
      ...
      } else {    // element is too small to use the interior ones, use the corner points
        int sign[2];
        sign[0] = -1;
        sign[1] = 1;
        int corner_limit = 8;
        //compute corner contributions (surface count = numAtomInUnitCell?)
        for (int sc = 0; sc < corner_limit; sc++) {

          if (sc == 0) {
            s = -1 + (0.5) * unit_cell_mapped[0];    //unit cell mapped = 2/numUnitCells
            t = -1 + (0.5) * unit_cell_mapped[1];
            w = -1 + (0.5) * unit_cell_mapped[2];
          } else if (sc == 1) {
            s = 1 - (0.5) * unit_cell_mapped[0];
            t = -1 + (0.5) * unit_cell_mapped[1];
            w = -1 + (0.5) * unit_cell_mapped[2];
          } else if (sc == 2) {
            s = 1 - (0.5) * unit_cell_mapped[0];
          ...
          //stw maxed out at 0.875, 0.875, 0.875 (with surf count 2)
          //printf("s: %f, t: %f, w: %f\n", s, t, w);
          quadrature_point_data[qi][0] = s;
          quadrature_point_data[qi][1] = t;
          quadrature_point_data[qi][2] = w;
  ...
  } else if (element_type[element_index] == 4) {    //Q4 quadrature point selection
    //check if refined numerical interface is needed
    if (atom->full_quad_flag && interface_flags[element_index]) {
      //loop over all possible virtual atom sites
      for (int i = 0; i < current_element_scale[0]; i++) {
        for (int j = 0; j < current_element_scale[1]; j++) {
          ...
          quadrature_point_data[qi][6] = unit_cell_mapped[0] * unit_cell_mapped[1];
          if (i == 0 && j == 0)
            quadrature_point_data[qi][7] = 1;
          else if (i == current_element_scale[0] - 1 && j == 0)
            quadrature_point_data[qi][7] = 1;
        ...
        for (int sc = 0; sc < s_count; sc++) {
          for (int i = 0; i < isurface_counts[0]; i++) {
            for (int j = 0; j < quadrature_node_count; j++) {
              ...
              quadrature_point_data[qi][5] = 0;
              quadrature_point_data[qi][6] =
                  unit_cell_mapped[0] * interior_scale[1] * quadrature_weights[j];
              quadrature_point_data[qi][7] = 0;
              neigh_quad_counter = neigh_quad_counter + 1;
              qi += 1;
              if (qi == quadrature_point_max) grow_quad_data();
...
int NPairCAC::CAC_decide_element2element(int origin_element_index, int neighbor_element_index)
{
  ...
  if (neighboring_element_type != 0) {
    ...
    for (int idim = 0; idim < domain->dimension; idim++) {
      if (bounding_boxlo[idim] >= neighbounding_boxlo[idim] &&
          bounding_boxlo[idim] <= neighbounding_boxhi[idim])
        box_overlap_flag[idim] = 1;
      if (bounding_boxhi[idim] >= neighbounding_boxlo[idim] &&
          bounding_boxhi[idim] <= neighbounding_boxhi[idim])
        box_overlap_flag[idim] = 1;
      if (bounding_boxlo[idim] <= neighbounding_boxlo[idim] &&
          bounding_boxhi[idim] >= neighbounding_boxhi[idim])
        box_overlap_flag[idim] = 1;
    ...
    box_overlap_flag[2] = box_overlap_flag[1] = box_overlap_flag[0] = 0;
    for (int idim = 0; idim < domain->dimension; idim++) {
      if (x[neighbor_element_index][idim] >= bounding_boxlo[idim] &&
          x[neighbor_element_index][idim] <= bounding_boxhi[idim])
        box_overlap_flag[idim] = 1;
    }
...
void NPairCAC::allocate_neigh_list()
{
  ...
  if (!neigh_allocated) {
    atom->list_container = list_container =
        (int **) memory->smalloc(atom->nlocal * sizeof(int *), "NPair CAC:list_container");
    memory->create(list_maxes, atom->nlocal, "NPair CAC:list_maxes");
    for (int init = 0; init < atom->nlocal; init++) {
      list_container[init] =
          memory->create(list_container[init], maxneigh, "NPair CAC:cell_indexes");
    }
  ...
  if (neigh_allocated && atom->nlocal > max_atom_count) {
    atom->list_container = list_container = (int **) memory->srealloc(
        list_container, atom->nlocal * sizeof(int *), "NPair CAC:list_container");
  ...
  neigh_allocated = 1;
  if (atom->nlocal > max_atom_count) max_atom_count = atom->nlocal;
}
...
void NPairCAC::allocate_local_arrays()
{
  if (!ghost_quad_flag) {
    ...
    atom->interior_scales =
        memory->grow(interior_scales, atom->nlocal, 3, "NPairCAC:interior_scales");
    atom->quadrature_counts =
        memory->grow(quadrature_counts, atom->nlocal, "NPair CAC:quadrature_counts");
    atom->e2quad_index = memory->grow(e2quad_index, atom->nlocal, "NPair CAC:e2quad_index");
    atom->neighbor_weights =
        memory->grow(neighbor_weights, atom->nlocal, 3, "NPair CAC:neighbor_weights");
    nmax = atom->nlocal;
  } else {
    atom->surface_counts =
        memory->grow(surface_counts, atom->nlocal + atom->nghost, 3, "NPairCAC:surface_counts");
    atom->interior_scales =
        memory->grow(interior_scales, atom->nlocal + atom->nghost, 3, "NPairCAC:interior_scales");
    atom->quadrature_counts =
        memory->grow(quadrature_counts, atom->nlocal + atom->nghost, "NPair CAC:quadrature_counts");
    atom->e2quad_index =
        memory->grow(e2quad_index, atom->nlocal + atom->nghost, "NPair CAC:e2quad_index");
    atom->neighbor_weights = memory->grow(neighbor_weights, atom->nlocal + atom->nghost, 3,
                                          "NPair CAC:neighbor_weights");
...
void NPairCAC::allocate_quad_neigh_list()
{
  ...
  for (int sum = 0; sum < sum_limit; sum++) quad_count += poly_count[sum] * quadrature_counts[sum];
  // initialize quadrature point neighbor list arrays
  if (!quad_allocated) {
    atom->inner_quad_lists_ucell = inner_quad_lists_ucell = (double ***) memory->smalloc(
        sizeof(double **) * quad_count, "NPair CAC:inner_quad_lists_ucell");
    atom->inner_quad_lists_index = inner_quad_lists_index =
        (int ***) memory->smalloc(sizeof(int **) * quad_count, "NPair CAC:inner_quad_lists_index");
    atom->inner_quad_lists_counts =
        memory->create(inner_quad_lists_counts, quad_count, "NPair CAC:inner_quad_lists_counts");
    atom->inner_quad_neigh_maxes =
        memory->create(inner_quad_neigh_maxes, quad_count, "NPair CAC:inner_quad_neigh_maxes");
    for (int neigh_loop = 0; neigh_loop < quad_count; neigh_loop++) {
      memory->create(inner_quad_lists_ucell[neigh_loop], maxneigh_quad_inner, 3,
                     "NPair CAC:inner_quad_lists_ucell");
      if (sector_flag)
        memory->create(inner_quad_lists_index[neigh_loop], maxneigh_quad_inner, 3,
                       "NPair CAC:inner_quad_lists_index");
      else
        memory->create(inner_quad_lists_index[neigh_loop], maxneigh_quad_inner, 2,
                       "NPair CAC:inner_quad_lists_index");
    }
    if (outer_neigh_flag) {
      atom->outer_quad_lists_ucell = outer_quad_lists_ucell = (double ***) memory->smalloc(
          sizeof(double **) * quad_count, "NPair CAC:outer_quad_lists_ucell");
      atom->outer_quad_lists_index = outer_quad_lists_index = (int ***) memory->smalloc(
          sizeof(int **) * quad_count, "NPair CAC:outer_quad_lists_index");
      atom->outer_quad_lists_counts =
          memory->create(outer_quad_lists_counts, quad_count, "NPair CAC:outer_quad_lists_counts");
      atom->outer_quad_neigh_maxes =
          memory->create(outer_quad_neigh_maxes, quad_count, "NPair CAC:outer_quad_neigh_maxes");
      for (int neigh_loop = 0; neigh_loop < quad_count; neigh_loop++) {
        memory->create(outer_quad_lists_ucell[neigh_loop], maxneigh_quad_outer, 3,
                       "NPair CAC:outer_quad_lists_ucell");
        if (sector_flag)
          memory->create(outer_quad_lists_index[neigh_loop], maxneigh_quad_outer, 3,
                         "NPair CAC:outer_quad_lists_index");
        else
          memory->create(outer_quad_lists_index[neigh_loop], maxneigh_quad_outer, 2,
                         "NPair CAC:outer_quad_lists_index");
      }
    }
    ...
    for (int init = 0; init < quad_count; init++) {
      ...
      if (outer_neigh_flag) { outer_quad_neigh_maxes[init] = maxneigh_quad_outer; }
    }
  }

  //grow quadrature point neighbor list arrays if needed
  if (quad_allocated && quad_count > quad_count_max) {
    atom->inner_quad_lists_ucell = inner_quad_lists_ucell = (double ***) memory->srealloc(
        inner_quad_lists_ucell, sizeof(double **) * quad_count, "Pair CAC:inner_quad_lists_ucell");
    atom->inner_quad_lists_index = inner_quad_lists_index = (int ***) memory->srealloc(
        inner_quad_lists_index, sizeof(int **) * quad_count, "Pair CAC:inner_quad_lists_index");
    atom->inner_quad_lists_counts =
        memory->grow(inner_quad_lists_counts, quad_count, "Pair CAC:inner_quad_lists_counts");
    atom->inner_quad_neigh_maxes =
        memory->grow(inner_quad_neigh_maxes, quad_count, "Pair CAC:inner_quad_neigh_maxes");
    for (int neigh_loop = quad_count_max; neigh_loop < quad_count; neigh_loop++) {
      memory->create(inner_quad_lists_ucell[neigh_loop], maxneigh_quad_inner, 3,
                     "Pair CAC:inner_quad_lists_ucell");
      if (sector_flag)
        memory->create(inner_quad_lists_index[neigh_loop], maxneigh_quad_inner, 3,
                       "Pair CAC:inner_quad_lists_index");
      else
        memory->create(inner_quad_lists_index[neigh_loop], maxneigh_quad_inner, 2,
                       "Pair CAC:inner_quad_lists_index");
    }
    if (outer_neigh_flag) {
      atom->outer_quad_lists_ucell = outer_quad_lists_ucell =
          (double ***) memory->srealloc(outer_quad_lists_ucell, sizeof(double **) * quad_count,
                                        "NPair CAC:outer_quad_lists_ucell");
      atom->outer_quad_lists_index = outer_quad_lists_index = (int ***) memory->srealloc(
          outer_quad_lists_index, sizeof(int **) * quad_count, "NPair CAC:outer_quad_lists_index");
      atom->outer_quad_lists_counts =
          memory->grow(outer_quad_lists_counts, quad_count, "NPair CAC:outer_quad_lists_counts");
      atom->outer_quad_neigh_maxes =
          memory->grow(outer_quad_neigh_maxes, quad_count, "NPair CAC:outer_quad_neigh_maxes");
      for (int neigh_loop = quad_count_max; neigh_loop < quad_count; neigh_loop++) {
        memory->create(outer_quad_lists_ucell[neigh_loop], maxneigh_quad_outer, 3,
                       "NPair CAC:outer_quad_lists_ucell");
        if (sector_flag)
  ...
  if (!add_quad_allocated && add_neigh_flag) {
    atom->add_quad_lists_ucell = add_quad_lists_ucell = (double ***) memory->smalloc(
        sizeof(double **) * quad_count, "NPair CAC:add_quad_lists_ucell");
    atom->add_quad_lists_index = add_quad_lists_index =
        (int ***) memory->smalloc(sizeof(int **) * quad_count, "NPair CAC:add_quad_lists_index");
    atom->add_quad_lists_counts =
        memory->create(add_quad_lists_counts, quad_count, "NPair CAC:add_quad_lists_counts");
    atom->add_quad_neigh_maxes =
        memory->create(add_quad_neigh_maxes, quad_count, "NPair CAC:add_quad_neigh_maxes");
    for (int neigh_loop = 0; neigh_loop < quad_count; neigh_loop++) {
      memory->create(add_quad_lists_ucell[neigh_loop], maxneigh_quad_add, 3,
                     "NPair CAC:add_quad_lists_ucell");
      if (sector_flag)
        memory->create(add_quad_lists_index[neigh_loop], maxneigh_quad_add, 3,
                       "NPair CAC:add_quad_lists_index");
      else
        memory->create(add_quad_lists_index[neigh_loop], maxneigh_quad_add, 2,
                       "NPair CAC:add_quad_lists_index");
    }

    //initialize count maxes
    for (int init = 0; init < quad_count; init++) add_quad_neigh_maxes[init] = maxneigh_quad_add;
  }

  //grow quadrature point neighbor list arrays if needed
  if (add_quad_allocated && quad_count > add_quad_count_max) {
    atom->add_quad_lists_ucell = add_quad_lists_ucell = (double ***) memory->srealloc(
        add_quad_lists_ucell, sizeof(double **) * quad_count, "NPair CAC:add_quad_lists_ucell");
    atom->add_quad_lists_index = add_quad_lists_index = (int ***) memory->srealloc(
        add_quad_lists_index, sizeof(int **) * quad_count, "NPair CAC:add_quad_lists_index");
    atom->add_quad_lists_counts =
        memory->grow(add_quad_lists_counts, quad_count, "NPair CAC:add_quad_lists_counts");
    atom->add_quad_neigh_maxes =
        memory->grow(add_quad_neigh_maxes, quad_count, "NPair CAC:add_quad_neigh_maxes");
    for (int neigh_loop = quad_count_max; neigh_loop < quad_count; neigh_loop++) {
      memory->create(add_quad_lists_ucell[neigh_loop], maxneigh_quad_add, 3,
                     "NPair CAC:add_quad_lists_ucell");
      if (sector_flag)
  ...
  for (int init = 0; init < quad_count; init++) {
    ...
    if (outer_neigh_flag) outer_quad_lists_counts[init] = 0;
    if (add_neigh_flag) add_quad_lists_counts[init] = 0;
  }
  quad_allocated = 1;
  if (add_neigh_flag) add_quad_allocated = 1;
  if (quad_count > quad_count_max) quad_count_max = quad_count;

  if (add_neigh_flag)
    if (quad_count > add_quad_count_max) add_quad_count_max = quad_count;
}
...
int NPairCAC::quad_sector_select(double s, double t, double w, int eindex, int pindex)
{
  ...
  int quad_index = 0;
  //Q8 implementation
  if (element_type == 1 || element_type == 3) {
    double unit_cell_mapped[3];
    ...
    if (quadrature_node_count != 2)
      error->one(FLERR,
                 "Remember to generalize the quadrature proximity calculation in "
                 "quad_sector_select in pair_cac.cpp (only quad=2 now)");
    //interior cell point
    ...
    if (surfs != 0 && surft == 0 && surfw == 0) {
      if (surfs == -1)
        quad_index =
            quad * quad * quad + quad * quad * s1 + quad * (signt + 1) / 2 + (signw + 1) / 2;
      if (surfs == 1)
        quad_index = quad * quad * quad + quad * quad * n1 + quad * quad * s1 +
            quad * (signt + 1) / 2 + (signw + 1) / 2;
      return quad_index;
    }
    //if t surface point
    if (surfs == 0 && surft != 0 && surfw == 0) {
      if (surft == -1)
        quad_index = quad * quad * quad + 2 * quad * quad * n1 + quad * quad * s2 +
            quad * (signs + 1) / 2 + (signw + 1) / 2;
      if (surft == 1)
        quad_index = quad * quad * quad + 2 * quad * quad * n1 + quad * quad * n2 +
            quad * quad * s2 + quad * (signs + 1) / 2 + (signw + 1) / 2;
      return quad_index;
    }
    //if w surface point
    if (surfs == 0 && surft == 0 && surfw != 0) {
      if (surfw == -1)
        quad_index = quad * quad * quad + 2 * quad * quad * (n1 + n2) + quad * quad * s3 +
            quad * (signs + 1) / 2 + (signt + 1) / 2;
      if (surfw == 1)
        quad_index = quad * quad * quad + 2 * quad * quad * (n1 + n2) + quad * quad * n3 +
            quad * quad * s3 + quad * (signs + 1) / 2 + (signt + 1) / 2;
      return quad_index;
    }
    //if w oriented edge point
    if (surfs != 0 && surft != 0 && surfw == 0) {
      quad_index = quad * quad * quad + 2 * quad * quad * (n1 + n2 + n3);
      if (signs == -1 && signt == -1) quad_index += n2 * quad * s1 + quad * s2 + (signw + 1) / 2;
      if (signs == 1 && signt == -1)
    ...
    if (surfs == 0 && surft != 0 && surfw != 0) {
      ...
      if (signw == -1 && signt == -1) quad_index += n3 * quad * s2 + quad * s3 + (signs + 1) / 2;
      if (signw == -1 && signt == 1)
    ...
    if (surfs != 0 && surft == 0 && surfw != 0) {
      quad_index = quad * quad * quad + 2 * quad * quad * (n1 + n2 + n3) + 4 * quad * n1 * n2 +
          4 * quad * n2 * n3;
      if (signw == -1 && signt == -1) quad_index += n3 * quad * s1 + quad * s1 + (signt + 1) / 2;
      if (signw == -1 && signs == 1)
    ...
    quad_index = quad * quad * quad + 2 * quad * quad * (n1 + n2 + n3) +
        4 * quad * (n1 * n2 + n2 * n3 + n1 * n3);
    //check for corner points
    if (surfs == -1 && surft == -1 && surfw == -1) quad_index += n3 * n2 * s1 + n3 * s2 + s3;
    if (surfs == 1 && surft == -1 && surfw == -1)
      quad_index += n1 * n2 * n3 + n3 * n2 * s1 + n3 * s2 + s3;
    if (surfs == 1 && surft == 1 && surfw == -1)
      quad_index += 2 * n1 * n2 * n3 + n3 * n2 * s1 + n3 * s2 + s3;
    if (surfs == -1 && surft == 1 && surfw == -1)
      quad_index += 3 * n1 * n2 * n3 + n3 * n2 * s1 + n3 * s2 + s3;
    if (surfs == -1 && surft == -1 && surfw == 1)
      quad_index += 4 * n1 * n2 * n3 + n3 * n2 * s1 + n3 * s2 + s3;
    if (surfs == 1 && surft == -1 && surfw == 1)
      quad_index += 5 * n1 * n2 * n3 + n3 * n2 * s1 + n3 * s2 + s3;
    if (surfs == 1 && surft == 1 && surfw == 1)
      quad_index += 6 * n1 * n2 * n3 + n3 * n2 * s1 + n3 * s2 + s3;
    if (surfs == -1 && surft == 1 && surfw == 1)
      quad_index += 7 * n1 * n2 * n3 + n3 * n2 * s1 + n3 * s2 + s3;
    return quad_index;
  }
  return quad_index;
}
...
void NPairCAC::grow_quad_data()
{
  ...
  atom->quadrature_point_data =
      memory->grow(quadrature_point_data, quadrature_point_max, 8, "pairCAC:quadrature_point_data");
...
void NPairCAC::allocate_quad_int(int **&int_quad_array, int size)
{
  if (quadrature_poly_count > quadrature_poly_max) {
    ...
    int_quad_array =
        memory->grow(int_quad_array, quadrature_poly_max, size, "pairCAC:int_quad_array");
  }
}
...
void NPairCAC::allocate_quad_double(double **&double_quad_array, int size)
{
  if (quadrature_poly_count > quadrature_poly_max) {
    quadrature_poly_max = quadrature_poly_count;
    double_quad_array =
        memory->grow(double_quad_array, quadrature_poly_max, size, "pairCAC:double_quad_array");
  }
}
...
int NPairCAC::pack_forward_comm(int n, int *list, double *buf, int /*pbc_flag*/, int * /*pbc*/)
{
  int i, j, m;
...
void NPairCAC::unpack_forward_comm(int n, int first, double *buf)
{
  ...
  m = 0;
  last = first + n;
  for (i = first; i < last; i++) { interface_flags[i] = buf[m++]; }
}
