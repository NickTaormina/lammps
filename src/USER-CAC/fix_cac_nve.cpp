...
FixNVECAC::FixNVECAC(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
...
void FixNVECAC::initial_integrate(int vflag)
{
  ...
  int **connected_nodes = atom->connected_nodes;
  int *connected_nodes_count = atom->connected_nodes_count;
      ...
      if (mask[i] & groupbit) {
        ...
        for (int poly_counter = 0; poly_counter < poly_count[i]; poly_counter++) {
          for (int k = 0; k < nodes_per_element; k++) {
            ...
            int *connected = connected_nodes[i];
        ...
        int *connected = connected_nodes[i];
        double x_avg[3] = {0, 0, 0};
        double v_avg[3] = {0, 0, 0};
        double f_avg[3] = {0, 0, 0};
        for (int j = 0; j < 1; j++) {
          //printf("%d ", connected[j]);
          x_avg[0] += x[connected[j]][0];
          x_avg[1] += x[connected[j]][1];
          x_avg[2] += x[connected[j]][2];
          v_avg[0] += v[connected[j]][0];
          v_avg[1] += v[connected[j]][1];
          v_avg[2] += v[connected[j]][2];
          f_avg[0] += f[connected[j]][0];
          f_avg[1] += f[connected[j]][1];
          f_avg[2] += f[connected[j]][2];
        }
        x_avg[0] = x_avg[0] / nodes_per_element;
        x_avg[1] = x_avg[1] / nodes_per_element;
        x_avg[2] = x_avg[2] / nodes_per_element;
        v_avg[0] = v_avg[0] / nodes_per_element;
        v_avg[1] = v_avg[1] / nodes_per_element;
        v_avg[2] = v_avg[2] / nodes_per_element;
        f_avg[0] = f_avg[0] / nodes_per_element;
        f_avg[1] = f_avg[1] / nodes_per_element;
        f_avg[2] = f_avg[2] / nodes_per_element;
