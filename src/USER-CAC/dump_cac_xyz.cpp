...
DumpCACXYZ::DumpCACXYZ(LAMMPS *lmp, int narg, char **arg) : Dump(lmp, narg, arg), typenames(NULL)
{
...
void DumpCACXYZ::init_style()
{
  ...
  if (buffer_flag == 1)
    write_choice = &DumpCACXYZ::write_string;
  else
    write_choice = &DumpCACXYZ::write_lines;
