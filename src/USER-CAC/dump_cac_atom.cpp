...
DumpCACAtom::DumpCACAtom(LAMMPS *lmp, int narg, char **arg) : Dump(lmp, narg, arg), typenames(NULL)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "charge") == 0)
      ...
      error->all(FLERR, "Unexpected argument in dump cac/atom command");
    iarg++;
  }
...
void DumpCACAtom::init_style()
{
  ...
  if (buffer_flag == 1)
    write_choice = &DumpCACAtom::write_string;
  else
    write_choice = &DumpCACAtom::write_lines;
...
int DumpCACAtom::convert_string(int n, double *mybuf)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (charge_flag)
      offset +=
          sprintf(&sbuf[offset], format, static_cast<int>(mybuf[m]), static_cast<int>(mybuf[m + 1]),
                  mybuf[m + 2], mybuf[m + 3], mybuf[m + 4], mybuf[m + 5], mybuf[m + 6],
                  mybuf[m + 7], mybuf[m + 8], mybuf[m + 9], mybuf[m + 10], mybuf[m + 11]);
    else
      offset +=
          sprintf(&sbuf[offset], format, static_cast<int>(mybuf[m]), static_cast<int>(mybuf[m + 1]),
                  mybuf[m + 2], mybuf[m + 3], mybuf[m + 4], mybuf[m + 5], mybuf[m + 6],
