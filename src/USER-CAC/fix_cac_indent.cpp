...
FixCACIndent::FixCACIndent(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  xstr(nullptr), ystr(nullptr), zstr(nullptr), rstr(nullptr), pstr(nullptr)
{
  if (narg < 4) error->all(FLERR,"Illegal fix cac/indent command");
  ...
  } else error->all(FLERR,"Illegal fix cac/indent command");
...
void FixCACIndent::init()
{
  if (xstr) {
    xvar = input->variable->find(xstr);
    if (xvar < 0)
      error->all(FLERR,"Variable name for fix cac/indent does not exist");
    if (!input->variable->equalstyle(xvar))
      error->all(FLERR,"Variable for fix cac/indent is invalid style");
  }
  if (ystr) {
    ...
    if (!input->variable->equalstyle(yvar))
      error->all(FLERR,"Variable for fix cac/indent is not equal style");
  }
  if (zstr) {
...
void FixCACIndent::setup(int vflag)
{
...
void FixCACIndent::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixCACIndent::post_force(int /*vflag*/)
{
  ...
  if (istyle == SPHERE) {
    ...
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        ...
        delx = nodal_positions[i][l][j][0] - ctr[0];
        dely = nodal_positions[i][l][j][1] - ctr[1];
        delz = nodal_positions[i][l][j][2] - ctr[2];							 
        ...
        fy = dely*fmag/r;
        fz = delz*fmag/r;
        nodal_forces[i][l][j][0] += fx;
        nodal_forces[i][l][j][1] += fy;
        nodal_forces[i][l][j][2] += fz;
  ...
  // planar indenter
    ...
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
	    ...
	    for (int l = 0; l < poly_count[i]; l++) {
	  	for (int j = 0; j < nodes_per_element; j++) {
        dr = planeside * (plane - nodal_positions[i][l][j][cdim]);
        if (dr >= 0.0) continue;
        fatom = -planeside * k*dr*dr;
//        f[i][cdim] += fatom;
        nodal_forces[i][l][j][cdim] =nodal_forces[i][l][j][cdim]-planeside * k*dr*dr;
        indenter[0] -= k3 * dr*dr*dr;
...
void FixCACIndent::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixCACIndent::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixCACIndent::compute_scalar()
{
...
double FixCACIndent::compute_vector(int n)
{
  // only sum across procs one time
...
void FixCACIndent::options(int narg, char **arg)
{
  if (narg < 0) error->all(FLERR,"Illegal fix cac/indent command");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"sphere") == 0) {
      if (iarg+5 > narg) error->all(FLERR,"Illegal fix cac/indent command");
      ...
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix cac/indent command");
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix cac/indent command");
