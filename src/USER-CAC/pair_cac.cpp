...
void PairCAC::compute(int eflag, int vflag)
{
  ...
  if (pre_force_flag) pre_force_densities();
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    if (element_type[i] == 1 || element_type[i] == 3) {
      ...
      for (mi = 0; mi < max_nodes_per_element; mi++) {
        for (mj = 0; mj < max_nodes_per_element; mj++) { mass_copy[mi][mj] = mass_matrix[mi][mj]; }
      }

      singular = LUPDecompose(mass_copy, max_nodes_per_element, 0.00000000000001, pivot);
      if (singular == 0) { error->one(FLERR, "LU matrix is degenerate"); }
    }
    atomic_flag = 0;
    ...
    current_x = x[i];
    if (eflag) { element_energy = 0; }
    //determine element type
    nodes_per_element = nodes_count_list[current_element_type];
    if (current_element_type == 0) { atomic_flag = 1; }
    ...
    if (atomic_flag) {
      ...
      compute_forcev(i);
      for (int dim = 0; dim < 3; dim++) { nodal_forces[i][0][0][dim] += force_column[0][dim]; }
    } else {
    ...
    //Tally energy
    if (evflag) ev_tally_full(i, 2 * element_energy, 0.0, fpair, delx, dely, delz);
        ...
        for (mi = 0; mi < nodes_per_element; mi++) {
          ...
          if (element_type[i] != 0) {
            for (int dim = 0; dim < 6; dim++)
              v[dim] *= (element_scale[i][0] * element_scale[i][1] * element_scale[i][2]) /
                  nodes_count_list[element_type[i]];
          }
...
void PairCAC::allocate()
{
  ...
  memory->create(mass_copy, max_nodes_per_element, max_nodes_per_element,
                 "pairCAC:copy_mass_matrix");
...
void PairCAC::settings(int narg, char **arg)
{
  ...
  if (narg == 1) {
    if (strcmp(arg[0], "one") == 0)
      atom->one_layer_flag = one_layer_flag = 1;
    else
      error->all(FLERR, "Unexpected argument in CAC pair style invocation");
  }
...
void PairCAC::check_existence_flags()
{
  ...
  if (!atom->CAC_flag) error->all(FLERR, "CAC Pair style requires a CAC atom style");
  //check if hybrid pair style is invoked
  if (force->pair_match("hybrid", 0, 0) != NULL)
    error->all(FLERR,
               "CAC Pair styles cannot be invoked with hybrid; also don't use the word hybrid in "
               "your style name to avoid this error");
...
void PairCAC::quadrature_init(int quadrature_rank)
{
  ...
  memory->create(shape_quad_result, max_nodes_per_element,
                 quadrature_node_count * quadrature_node_count * quadrature_node_count,
                 "pairCAC:shape_quad_result");
}
    ...
    for (int i = 0; i < quadrature_node_count; i++) {
      for (int j = 0; j < quadrature_node_count; j++) {
        for (k = 0; k < quadrature_node_count; k++) {
          shape_quad_result[ii][quadrature_node_count * quadrature_node_count * (i) +
                                quadrature_node_count * (j) + k] =
              shape_function(quadrature_abcissae[i], quadrature_abcissae[j], quadrature_abcissae[k],
                             2, ii + 1);
        }
      }
    }
    ...
    for (int j = 0; j < quadrature_node_count; j++) {
      for (int k = 0; k < quadrature_node_count; k++) {
        result += quadrature_weights[i] * quadrature_weights[j] * quadrature_weights[k] *
            shape_quad_result[ii][quadrature_node_count * quadrature_node_count * (i) +
                                  quadrature_node_count * (j) + k] *
            shape_quad_result[jj][quadrature_node_count * quadrature_node_count * (i) +
                                  quadrature_node_count * (j) + k];
      }
    }
  }
...
void PairCAC::compute_forcev(int iii)
{
  ...
  for (int js = 0; js < nodes_per_element; js++) {
    for (int jj = 0; jj < 3; jj++) { force_column[js][jj] = 0; }
  }

  //init virial projection
  if (atom->CAC_virial) {
    for (int jj = 0; jj < 6; jj++) {
      for (int js = 0; js < nodes_per_element; js++) { current_virial_projection[jj][js] = 0; }
    }
  }
  ...
  if (flux_compute && atom->cac_flux_flag == 2) {
    for (int jj = 0; jj < 24; jj++) {
      for (int js = 0; js < nodes_per_element; js++) { current_flux_projection[jj][js] = 0; }
    }
  }
  ...
  for (int quad_loop = 0; quad_loop < quadrature_counts[iii]; quad_loop++) {
    ...
    if (quad_flux_flag)
      for (int init = 0; init < 24; init++) flux_density[init] = 0;
    if (!atomic_flag) {
    ...
    if (t == 0) { printf("t is zero %d\n", quad_loop); }

    coefficients = quadrature_point_data[qi + quad_loop][6];
    if (!atomic_flag)
      force_densities(iii, s, t, w, coefficients, force_density[0], force_density[1],
                      force_density[2]);
    else
      force_densities(iii, current_x[0], current_x[1], current_x[2], coefficients, force_density[0],
                      force_density[1], force_density[2]);
    if (!atomic_flag) {
      for (int js = 0; js < nodes_per_element; js++) {
      ...
      if (quad_flux_flag && atom->cac_flux_flag == 1) {
        ...
        for (int jj = 0; jj < 24; jj++) nodal_fluxes[iii][poly_counter][ns][jj] = flux_density[jj];
      }
    } else {
      for (int jj = 0; jj < 3; jj++) { force_column[0][jj] = force_density[jj]; }
      if (atom->CAC_virial) {
        for (int jj = 0; jj < 6; jj++) { nodal_virial[iii][0][0][jj] = virial_density[jj]; }
      }
      if (quad_flux_flag) {
        for (int jj = 0; jj < 24; jj++) { nodal_fluxes[iii][0][0][jj] = flux_density[jj]; }
      }
    }
    if (quad_eflag) { element_energy += coefficients * quadrature_energy / iso_volume; }
  }
  ...
  if (atom->CAC_virial) {
    for (int jj = 0; jj < 6; jj++) {
      LUPSolve(mass_copy, pivot, current_virial_projection[jj], nodes_per_element,
               current_virial_column);
      for (int js = 0; js < nodes_per_element; js++) {
  ...
  if (flux_compute && atom->cac_flux_flag == 2) {
    for (int jj = 0; jj < 24; jj++) {
      LUPSolve(mass_copy, pivot, current_flux_projection[jj], nodes_per_element,
               current_flux_column);
      for (int js = 0; js < nodes_per_element; js++) {
...
void PairCAC::allocate_quad_memory()
{
  ...
  if (quad_flux_flag) {
    ...
    if (outer_neighflag)
      neigh_max_all = neigh_max_inner + neigh_max_outer + neigh_max_add;
    else
      neigh_max_all = neigh_max_inner + neigh_max_add;
  }

  if (neigh_max_inner > local_inner_max) {
    memory->grow(inner_neighbor_types, neigh_max_inner + EXPAND, "Pair_CAC:inner_neighbor_types");
    memory->grow(inner_neighbor_coords, neigh_max_inner + EXPAND, 3,
                 "Pair_CAC:inner_neighbor_coords");
    if (atom->q_flag)
  ...
  if (neigh_max_inner > vel_inner_max && quad_flux_flag) {
    memory->grow(inner_neighbor_velocities, neigh_max_inner + EXPAND, 3,
                 "Pair_CAC:inner_neighbor_velocities");
    vel_inner_max = neigh_max_inner + EXPAND;
  }
  if (outer_neighflag) {
    if (neigh_max_outer > local_outer_max) {
      memory->grow(outer_neighbor_coords, neigh_max_outer + EXPAND, 3,
                   "Pair_CAC:outer_neighbor_coords");
      memory->grow(outer_neighbor_types, neigh_max_outer + EXPAND, "Pair_CAC:outer_neighbor_types");
      if (atom->q_flag)
        memory->grow(outer_neighbor_charges, neigh_max_outer + EXPAND,
                     "Pair_CAC:outer_neighbor_charges");
      local_outer_max = neigh_max_outer + EXPAND;
    }
    if (neigh_max_outer > vel_outer_max && quad_flux_flag) {
      memory->grow(outer_neighbor_velocities, neigh_max_outer + EXPAND, 3,
                   "Pair_CAC:outer_neighbor_velocities");
  ...
  if (quad_flux_flag) {
    ...
    if (neigh_max_add > local_add_max) {
      ...
      memory->grow(add_neighbor_velocities, neigh_max_add + EXPAND, 3,
                   "Pair_CAC:add_neighbor_velocities");
      local_add_max = neigh_max_add + EXPAND;
    }

    //allocate line-plane intersection arrays
    if (neigh_max_all > local_all_max) {
      flux_neigh_nintercepts = memory->grow(flux_neigh_nintercepts, neigh_max_all + EXPAND,
                                            "Pair CAC:flux_neigh_nintercepts");
      flux_neigh_indices =
          (int **) memory->srealloc(flux_neigh_indices, (neigh_max_all + EXPAND) * sizeof(int *),
                                    "Pair CAC:flux_neigh_indices");
      nplane_intersects =
          (int **) memory->srealloc(nplane_intersects, (neigh_max_all + EXPAND) * sizeof(int *),
                                    "Pair CAC:nplane_intersects");
      flux_neigh_intercepts = (int ***) memory->srealloc(flux_neigh_intercepts,
                                                         (neigh_max_all + EXPAND) * sizeof(int **),
                                                         "Pair CAC:flux_neigh_intercepts");
      ...
      for (int alloc = 0; alloc < neigh_max_all + EXPAND; alloc++) {
        memory->grow(flux_neigh_indices[alloc], neigh_max_all + EXPAND,
                     "Pair CAC:flux_neigh_indices[alloc]");
        memory->grow(nplane_intersects[alloc], neigh_max_all + EXPAND,
                     "Pair CAC:nplane_intersects[alloc]");
        memory->grow(flux_neigh_intercepts[alloc], neigh_max_all + EXPAND, 2,
                     "Pair CAC:flux_neigh_intercepts[alloc]");
      }
...
void PairCAC::init_quad_arrays()
{
  ...
  for (int l = 0; l < neigh_max_inner; l++) {
    ...
    if (atom->q_flag) inner_neighbor_charges[l] = node_charges[element_index][poly_index];
  }
  ...
  if (outer_neighflag)
    for (int l = 0; l < neigh_max_outer; l++) {
      ...
      if (atom->q_flag) outer_neighbor_charges[l] = node_charges[element_index][poly_index];
    }
  if (quad_flux_flag)
    for (int l = 0; l < neigh_max_add; l++) {
      ...
      if (atom->q_flag) add_neighbor_charges[l] = node_charges[element_index][poly_index];
    }
}
...
void PairCAC::interpolation(int iii, double sq, double tq, double wq)
{
  ...
  if (neigh_max > MAXNEIGHBUFF)
    error->one(FLERR,
               "increase MAXNEIGHBUFF in pair_cac.cpp; this overestimates the maximum possible "
               "neighbors for a quadrature point");
  ...
  if (current_element_type == 1 || current_element_type == 3) {
    ...
    if (quad_flux_flag) node_velocities = nodal_velocities[iii][poly_counter];
  ...
  if (outer_neighflag) {
    ...
    for (int l = 0; l < outer_neigh_max; l++) {
      ...
      //Q8 interpolation scheme
      if (outer_etypes[l] == 1 || outer_etypes[l] == 3) {
        s = outer_ucells[l][0];
        t = outer_ucells[l][1];
        w = outer_ucells[l][2];
        sp = (1 + s) / 8;
  ...
  if (quad_flux_flag) {
    ...
    for (int l = 0; l < add_neigh_max; l++) {
      ...
      //Q8 interpolation scheme
      if (add_etypes[l] == 1 || add_etypes[l] == 3) {
        s = add_ucells[l][0];
        t = add_ucells[l][1];
        w = add_ucells[l][2];
        sp = (1 + s) / 8;
...
void PairCAC::compute_intersections()
{
  ...
  for (int ineigh = 0; ineigh < all_neigh; ineigh++) {
    ...
    for (int jneigh = 0; jneigh < all_neigh; jneigh++) nplane_intersects[ineigh][jneigh] = 0;
  }
  ...
  for (int ineigh = 0; ineigh < all_neigh; ineigh++) {
    ...
    for (int jneigh = ineigh + 1; jneigh < all_neigh; jneigh++) {
      ...
      for (int isl = 0; isl < 2 * domain->dimension; isl++) {
        ...
        if (isl % 2 == 0)
          planecoord = current_position[is] - box_size[is] / 2 + box_center[is];
        else
          planecoord = current_position[is] + box_size[is] / 2 + box_center[is];
        plane_limits[0][0] =
            current_position[dim1] - box_size[dim1] / 2 + box_center[dim1] - PLANE_EPSILON;
        plane_limits[0][1] =
            current_position[dim1] + box_size[dim1] / 2 + box_center[dim1] + PLANE_EPSILON;
        plane_limits[1][0] =
            current_position[dim2] - box_size[dim2] / 2 + box_center[dim2] - PLANE_EPSILON;
        plane_limits[1][1] =
            current_position[dim2] + box_size[dim2] / 2 + box_center[dim2] + PLANE_EPSILON;
        ...
        if ((proj < 0 && scan_position2[is] < planecoord) ||
            ((proj > 0) && scan_position2[is] > planecoord))
          intersection_flag = 0;
        ...
        if (intersection_point[dim1] <= plane_limits[0][0] ||
            intersection_point[dim1] > plane_limits[0][1])
          intersection_flag = 0;
        if (intersection_point[dim2] <= plane_limits[1][0] ||
            intersection_point[dim2] > plane_limits[1][1])
          intersection_flag = 0;
        ...
        if (nplane_intersects[ineigh][flux_neigh_nintercepts[ineigh]] == 2) break;
      }
      if (neigh_intersect) flux_neigh_nintercepts[ineigh]++;
    }
  }
}
...
void PairCAC::current_quad_flux(int jneigh, double interactionx, double interactiony,
                                double interactionz)
{
  ...
  for (int isl = 0; isl < 2 * domain->dimension; isl++) {
    ...
    //test if i-j normal coordinates are on opposing sides of the plane
    if ((proj < 0 && xj[is] < planecoord) || ((proj > 0) && xj[is] > planecoord))
      intersection_flag = 0;
    ...
    if (intersection_flag) {
      ...
      */
      ...
      if (flux_enable == 1) {
        flux_density[4 * isl] +=
            (interaction_forceij[0] * (vix + vjx) + interaction_forceij[1] * (viy + vjy) +
             interaction_forceij[2] * (viz + vjz)) *
            sign;
        ...
        flux_density[4 * isl] += (interaction_forceij[0] * vix + interaction_forceij[1] * viy +
                                  interaction_forceij[2] * viz) *
            sign;
        flux_density[4 * isl + 1] -= interaction_forceij[0] * sign;
        flux_density[4 * isl + 2] -= interaction_forceij[1] * sign;
        flux_density[4 * isl + 3] -= interaction_forceij[2] * sign;
...
void PairCAC::quad_neigh_flux()
{
  ...
  for (int ineigh = 0; ineigh < all_neigh; ineigh++) {
    ...
    for (int jneigh = ineigh + 1; jneigh < all_neigh; jneigh++) {
      ...
      if (flux_enable == 1) {
        if (atom->q_flag)
          fpair = pair_interaction_q(distancesq, scan_type1, scan_type2, q1, q2);
        else
          fpair = pair_interaction(distancesq, scan_type1, scan_type2);
      ...
      for (int isl = 0; isl < 2 * domain->dimension; isl++) {
        ...
        if (intersection_flag) {
          ...
          if (scan_position1[is] < planecoord)
            sign = -normal_flag;
          else
            sign = normal_flag;
          //flux_enable is 1 in the case of pair forces, 2 in the case of many-body
...
double PairCAC::shape_function_derivative(double s, double t, double w, int flag, int index,
                                          int derivative)
{
  double shape_function;
